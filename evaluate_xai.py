...
from config import (
    ...
    GLOBAL_MANIFEST, PSEUDO_DIR, GOLD_IMAGES_DIR,
    STUDENT_CKPT_DIR,
...
from train_student import StudentModel, StudentDataset, make_student_transforms


def build_gold_sample_list(mode: str) -> list[dict]:
    """
    Build the XAI evaluation sample list from the gold-standard set
    (data/gold_standard/images, 502 hand-annotated images), instead of the
    pseudo-labeled 'test' split. The pseudo-label factory is only ever run
    on train/val, so build_sample_list("test", mode) always returns empty —
    and even if it didn't, scoring XAI faithfulness metrics (pointing-game
    accuracy) against factory-generated pseudo-masks would be circular
    rather than a real held-out check. This mirrors validate_student.py,
    ...
    for img_path in sorted(GOLD_IMAGES_DIR.iterdir()):
        ...
        if sev_path.exists():
            ...
            n_skipped += 1
            continue  # no pseudo-mask/severity generated for this image

        wt_path = mode_dir / f"{original_stem}_weight.txt"
        weight  = float(wt_path.read_text().strip()) if wt_path.exists() else 1.0

        grade_path = mode_dir / f"{original_stem}_grade.txt"
    ...
    if n_skipped:
        print(f"  [INFO] {n_skipped} gold-standard image(s) skipped "
              f"(no matching pseudo-mask/severity files in {mode_dir})")
...
def get_target_layer(model: nn.Module, encoder_variant: str) -> nn.Module:
    ...
    # Resolve the layer string to an actual module, walking one path
    # component at a time so a bad component can be reported precisely.
    # Pattern: "unet.encoder.model.blocks[-1][-1]" etc.
    obj = model
    walked = []
    for part in layer_str.split("."):
        attr = part[:part.index("[")] if "[" in part else part
        try:
            obj = getattr(obj, attr)
        except AttributeError:
            available = [n for n, _ in obj.named_children()]
            raise AttributeError(
                f"Could not resolve target layer '{layer_str}' for "
                f"'{encoder_variant}': '{attr}' not found at "
                f"'{'.'.join(walked) or '<root>'}'. "
                f"Available children there: {available}"
            )
        walked.append(attr)
        ...
        if "[" in part:
            ...
            for idx_str in re.findall(r'\[(-?\d+)\]', indices):
                ...
                try:
                    ...
                    raise IndexError(
                        f"Could not resolve target layer '{layer_str}' for "
                        f"'{encoder_variant}': index [{idx}] invalid on "
                        f"'{walked[-1]}' (length {length}). Original error: {e}"
                    )
            walked[-1] = part
...
class _ClassifierOnlyView(nn.Module):
    """
    pytorch_grad_cam expects model(x) to return a single (batch, num_classes)
    tensor. StudentModel.forward returns (seg_logits, cls_out, sev_out), so
    passing it directly causes the CAM library's zip(targets, outputs) to
    silently pair the target with seg_logits (the first tuple element)
    instead of cls_out — producing a non-scalar loss and a cryptic
    ".backward()" crash with no indication cls_out was never used. This
    thin wrapper exposes only cls_out, while target_layer hooks still work
    since it references the same underlying submodule instance.
    """
...
class GradCAMWrapper:
    ...
    def __init__(self, model: nn.Module, target_layer: nn.Module,
        ...
        try:
            ...
            cls_only_model = _ClassifierOnlyView(model)
            self.cam    = cam_cls(model=cls_only_model, target_layers=[target_layer])
            self.method = method
...
def main() -> None:
    ...
    ckpt_filename = f"student_{best_variant}_{best_mode}_best.pth"

    # Checkpoints live under stage1/ or stage2/, not flat in STUDENT_CKPT_DIR.
    # mode_b is reused from Stage 1 (same seed => identical run, per the
    # two-stage ablation design) and was never re-saved in stage2/, so check
    # stage1/ first for mode_b and stage2/ first for every other mode —
    # then fall back to searching the other subfolder just in case.
    if best_mode == "mode_b":
        search_order = ["stage1", "stage2"]
    else:
        search_order = ["stage2", "stage1"]
    ...
    if ckpt_path is None:
        # Nothing found in either subfolder — report the primary expected
        # location so the error message still points somewhere sensible.
        ckpt_path = STUDENT_CKPT_DIR / search_order[0] / ckpt_filename
        print(f"[FATAL] Student checkpoint not found: {ckpt_path}")
        print(f"        Also checked: {STUDENT_CKPT_DIR / search_order[1] / ckpt_filename}")
    ...
    # ── Build evaluation dataset (gold-standard set, not pseudo-label 'test' split) ──
    test_samples = build_gold_sample_list(best_mode)
    if not test_samples:
        print("[FATAL] No gold-standard samples with matching pseudo-masks found.")
        return
    ...
    for sample_idx in selected_indices:
        ...
        img_rgb = load_image_rgb(sample["source_path"])
        if img_rgb is None:
            img_rgb = np.array(Image.open(sample["source_path"]).convert("RGB"))
