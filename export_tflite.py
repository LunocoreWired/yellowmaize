...
================================================================================
 ...
 PIPELINE:
   ...
   3. Convert ONNX → TFLite via onnx-tf and TensorFlow Lite converter
...
def convert_onnx_to_tflite(onnx_path: Path, tflite_path: Path) -> bool:
    """Convert ONNX → TFLite using onnx2tf."""
    try:
        import onnx2tf

        out_dir = tflite_path.parent / "onnx2tf_out"
        ...
        shutil.copy(target_file, tflite_path)
...
def main() -> None:
    ...
    if tflite_ok:
        ...
        print(f"         pip install onnx onnxruntime onnx-tf tensorflow")
