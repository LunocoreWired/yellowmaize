...
def export_bouncer_tflite() -> tuple[bool, float]:
    """
    Export Bouncer (MobileNetV3-Large binary classifier) to TFLite.
    ...
    try:
        ...
        tf_dir = DEPLOY_DIR / "_bouncer_tf_tmp"
        prepare(onnx.load(str(onnx_path))).export_graph(str(tf_dir))

        converter = tf.lite.TFLiteConverter.from_saved_model(str(tf_dir))
        converter.optimizations = [tf.lite.Optimize.DEFAULT]
        converter.target_spec.supported_types = [tf.float16]
        tflite_bytes = converter.convert()
        BOUNCER_TFLITE.write_bytes(tflite_bytes)
        ...
        print(f"  [WARN] TFLite deps missing: {e}")
        print("         pip install onnx-tf tensorflow")
        # Save PyTorch state dict as fallback reference
        torch.save(model.state_dict(),
                   DEPLOY_DIR / "bouncer_weights_fallback.pt")
        print("  Saved PyTorch weights as fallback.")
        return False, 0.0
    except Exception as e:
